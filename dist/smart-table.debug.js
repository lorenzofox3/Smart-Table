...
ng.module('smart-table')
  .controller('stTableController', ['$scope', '$parse', '$filter', '$attrs', function StTableController($scope, $parse, $filter, $attrs) {
    ...
    this.selectRange = function selectRange(start, end) {
      var rows = displayGetter($scope);
      var min = Math.min(rows.indexOf(start), rows.indexOf(end));
      var max = Math.max(rows.indexOf(start), rows.indexOf(end));
      var i;
      if (min !== -1) {
        for (i = min; i <= max; i++) {
